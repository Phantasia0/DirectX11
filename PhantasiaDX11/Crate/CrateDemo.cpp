...
bool CrateApp::Init()
{
	...
	{
		std::wstring fileName = L"FireAnim/Fire";
		...
		else if ((int(i / 10) >= 1) && ((int(i / 10)) < 10))
		{
			fileName += L"0";
			fileName += std::to_wstring(i);
		}
		...
		HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice,
			fileName.c_str(), 0, 0, &mDiffuseMapSRV[i-1], 0));
	}
...
void CrateApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		{
			if(float(1.0f / 30.0f) * (index -1) < time && time < float(1.0f / 30.0f) * index)

			Effects::BasicFX->SetDiffuseMap(mDiffuseMapSRV[index -1]);
		}
